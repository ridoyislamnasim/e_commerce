...
// import ReturnStatus from '../Select/ReturnStatus';
// import PaymentStatus from '../Select/PaymentStatus';
// import ReturnStatusAllParcel from './ReturnStatusAllParcel';
import PaymentStatusAllParcel from './PaymentStatusAllParcel';

const PrintAllParcel = ({ statusId, dataPending, dataForExport }) => {
    ...
    const productValueTotal = dataPending?.data?.result?.reduce((total, item) => {
        return total + parseFloat(item?.product_value || 0);
    }, 0);

    const collectedAmountTotal = dataPending?.data?.result?.reduce((total, item) => {
        return total + parseFloat(item?.customer_collect_amount || 0);
    }, 0);

    const codeChargeTotal = dataPending?.data?.result?.reduce((total, item) => {
        return total + parseFloat(item?.cod_charge || 0);
    }, 0);

    const totalChanrge = dataPending?.data?.result?.reduce((total, item) => {
        return total + parseFloat(item?.total_charge || 0);
    }, 0);


    // Data Print
    const productValueTotalDataPrint = dataForExport?.data?.result?.reduce((total, item) => {
        return total + parseFloat(item?.product_value || 0);
    }, 0);

    const collectedAmountTotalDataPrint = dataForExport?.data?.result?.reduce((total, item) => {
        return total + parseFloat(item?.customer_collect_amount || 0);
    }, 0);

    const codeChargeTotalDataPrint = dataForExport?.data?.result?.reduce((total, item) => {
        return total + parseFloat(item?.cod_charge || 0);
    }, 0);

    const totalChanrgeDataPrint = dataForExport?.data?.result?.reduce((total, item) => {
        return total + parseFloat(item?.total_charge || 0);
    }, 0);
    ...
    console.log("dataForExport", dataForExport?.data?.result)
                            ...
                            <div className=' text-gray-700'>
                                <table style={tableStyle}>
                                    ...
                                    <tbody>
                                        ...
                                        {
                                            dataPending?.data?.result?.map((details, index) => (
                                                <tr key={details?.id}>
                                        ...
                                        <tr>
                                            ...
                                            <td className='text-[11px]' style={tdStyle}> {parseFloat(productValueTotal).toFixed(2)} </td>
                                            <td className='text-[11px]' style={tdStyle}> {parseFloat(collectedAmountTotal).toFixed(2)} </td>
                                            <td className='text-[11px]' style={tdStyle}> {parseFloat(codeChargeTotal).toFixed(2)} </td>
                                            <td className='text-[11px]' style={tdStyle}> {parseFloat(totalChanrge).toFixed(2)} </td>
                                            <td className='text-[11px]' style={tdStyle}></td>
                                            <td className='text-[11px]' style={tdStyle}></td>
                                            <td className='text-[11px]' style={tdStyle}></td>
                                        </tr>
                                ...
                                <div className=' text-gray-700'>
                                    <table style={tableStyle}>
                                        ...
                                        <tbody>
                                            ...
                                            {
                                                dataForExport?.data?.result?.map((details, index) => (
                                                    <tr key={details?.id}>
                                                        <td className='text-[5px]' style={tdStyle}>{index + 1}</td>
                                                        <td className='text-[5px]' style={tdStyle}>{details?.parcel_invoice}</td>
                                                        <td className='text-[5px]' style={tdStyle}>{details?.merchant_order_id}</td>
                                                        ...
                                                        <td className='text-[5px]' style={tdStyle}>
                                                            {/* {
                                                                details?.status === 26 ? <p>Return Transfer</p> :
                                                                    details?.status === 27 ? <p>Return Transfer Cancel</p> :
                                                                        details?.status === 28 ? <p>Return Transfer Complete</p> :
                                                                            details?.status === 29 ? <p>Return Transfer Reject</p> :
                                                                                details?.status === 30 ? <p>Assign For Return</p> :
                                                                                    details?.status === 31 ? <p>Assign For Return start</p> :
                                                                                        details?.status === 32 ? <p>Return Run Cancel</p> :
                                                                                            details?.status === 33 ? <p>On The Way To Return</p> :
                                                                                                details?.status === 34 ? <p>Return Run Rider Reject</p> :
                                                                                                    details?.status === 35 ? <p>Rider Returned</p> :
                                                                                                        details?.status === 36 ? <p>Returned </p> : null
                                                            } */}
                                                            <ReturnStatusAllParcel
                                                                status={details?.status}
                                                                delivery_type={details?.delivery_type}
