...
export const CustomTable: React.FC<Props> = ({ data, onSubmit, onDelete, fetchData, pagination, onPageChange }) => {
  ...
  const columns: ColumnDef<TCategory, any>[] = [
    {
      id: "select",
      header: ({ table }: HeaderContext<TCategory, any>) => (
        <input
          type="checkbox"
    ...
    },
    {
      id: "sl",
      header: "SL",
      cell: ({ row }: { row: Row<TCategory> }) => row.index + 1,
    },
    {
      id: "name",
      ...
      cell: ({ row }: { row: Row<TCategory> }) => {
        ...
        return (
          <div className="w-20 h-20 relative">
            <PreviewImage src={image} alt="category" />
          </div>
        );
      },
    },
