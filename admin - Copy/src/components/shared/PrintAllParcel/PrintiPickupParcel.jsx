...
const PrintiPickupParcel = ({ pickupParcelData, togglePrintClick }) => {
    console.log("pickupParcelData", pickupParcelData)
    ...
    const totalRunParcel = pickupParcelData?.data?.result?.reduce((total, item) => {
        return total + parseFloat(item?.total_run_parcel || 0);
    }, 0);

    const totalRunCompleteParcel = pickupParcelData?.data?.result?.reduce((total, item) => {
            ...
            <div className='hidden '>
                <div className='w-full mt-10 text-gray-700 ' ref={componentRef} >
                    ...
                    <div className=' flex flex-col text-center items-center justify-center'>
                        ...
                        <p className='text-xl font-semibold mt-1'>Pickup Rider Run List</p>
                                ...
                                <table style={tableStyle}>
                                    <thead>
                                        <tr>
                                            ...
                                            <th className='font-bold  text-[10px] ' style={thStyle}>Rider Name</th>
                                            <th className='font-bold  text-[10px] ' style={thStyle}>Rider Phone</th>
                                            <th className='font-bold  text-[10px] ' style={thStyle}>Create Date</th>
                                            <th className='font-bold  text-[10px] ' style={thStyle}>Complete Date</th>
                                            {/* <th className='font-bold  text-[10px] ' style={thStyle}>Total Amount </th> */}
                                            <th className='font-bold  text-[10px] ' style={thStyle}>Run Parcel</th>
                                            <th className='font-bold  text-[10px] ' style={thStyle}>Complete Parcel</th>
                                    ...
                                    <tbody>
                                        ...
                                        {
                                            pickupParcelData?.data?.result?.map((details, index) => (
                                                <tr key={details?.id}>
...
export default PrintiPickupParcel;