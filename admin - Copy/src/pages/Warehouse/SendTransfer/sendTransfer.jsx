...
import { useGetAllTransferFromByPaginationQuery } from '@/store/api/app/Transfer/transferApiSlice';

const SendTransfer = () => {
  const [paginationPage, setPaginationPage] = useState(1);
  const [selectedRows, setSelectedRows] = useState([]);
  const [limit, setLimit] = useState(10);
  const [sendTransfer, setSendTransfer] = useState('desc');
  const noImage = useNoImage();
  const { isAuth, auth } = useSelector((state) => state.auth);
  // Fetch sendTransfers data

  const { data, isLoading: isSendTransfersLoading, isError, error } =
  useGetAllTransferFromByPaginationQuery({
      page: paginationPage,
      limit: limit,
      // sendTransfer: sendTransfer,
      idFrom: auth?.user?.warehouseRef,
    });
  ...
  const { data: roleData, isLoading: isRoleLoading } = useGetSingleRoleByIdQuery({
    ...
    permission: 'sendTransfer',
  });
  const id = auth?.user?.roleRef

  // Memoize permissions to avoid unnecessary re-renders
  // const permission = useMemo(() => roleData?.data?.permissions?.sendTransfer, [roleData]);
  const permission = {
    "access": true,
    "create": true,
    "edit": true,
  ...
  if (isSendTransfersLoading || isRoleLoading) {
  ...
  return (
    <>
      <CustomPaginationTable
        ...
        setLimit={setLimit}
        sendTransfer={sendTransfer}
        setSendTransfer={setSendTransfer}
        defaultStatus={false}
        defaultAction={false}
        // isView={false}
        // isEdit={false}
