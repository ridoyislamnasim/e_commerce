...
import { useGetChildCategorysByPaginationQuery } from '@/store/api/app/ChildCategory/childCategoryApiSlice';
...
const ChildCategory = () => {
  ...
  const { data, isLoading: isChildCategorysLoading, isError, error } =
  ...
  const { data: roleData, isLoading: isRoleLoading } = useGetSingleRoleByIdQuery({
    ...
    permission: 'chilsCategory',
  });

  // Memoize permissions to avoid unnecessary re-renders
  const permission = useMemo(() => roleData?.data?.permissions?.chilsCategory, [roleData]);

  // Check if data or permissions are still loading
  if (isChildCategorysLoading || isRoleLoading) {
