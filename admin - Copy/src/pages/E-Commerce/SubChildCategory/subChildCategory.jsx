...
import { useGetSubChildCategorysByPaginationQuery } from '@/store/api/app/SubChildCategory/subChildCategoryApiSlice';
...
const SubChildCategory = () => {
  ...
  const { data, isLoading: isSubChildCategorysLoading, isError, error } =
  ...
  const { data: roleData, isLoading: isRoleLoading } = useGetSingleRoleByIdQuery({
    ...
    permission: 'subChilsCategory',
  });

  // Memoize permissions to avoid unnecessary re-renders
  const permission = useMemo(() => roleData?.data?.permissions?.subChilsCategory, [roleData]);

  // Check if data or permissions are still loading
  if (isSubChildCategorysLoading || isRoleLoading) {
