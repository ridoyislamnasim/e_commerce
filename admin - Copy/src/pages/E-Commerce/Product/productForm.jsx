...
const ProductForm = ({ id, data }) => {
	...
	const [activeTabName, setActiveTabName] = useState(() => {
		// If editing and inventoryType exists, open the correct tab
		if (id && data?.inventoryType) {
			const matchedTab = buttons.find(
				(button) => button.name === data.inventoryType
			);
			...
			<Card title={id ? "Edit Product" : "Create New Product"}>
				<div className="grid grid-cols-1 gap-5">
					...
					<div className="flex gap-5"> 
						<label className="block text-md font-medium">Free Shipping</label>
						<input
									...
									{[0].map((index) => (
										<React.Fragment key={index}>
											<Textinput
												...
												required={true}
												error={errors?.inventoryArray?.[index]?.quantity}
											/>
											<Textinput
												register={register}
												label="MRP Price"
												...
												required={true}
												error={errors?.inventoryArray?.[index]?.mrpPrice}
											/>
											<Textinput
												register={register}
												label="Cost Price"
												...
												required={true}
												error={errors?.inventoryArray?.[index]?.costPrice}
											/>
										</React.Fragment>
									))}
									{/* <Textinput
