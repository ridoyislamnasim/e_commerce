...
import { useGetBrandsByPaginationQuery } from '@/store/api/app/Brand/categoryApiSlice';
...
const Brand = () => {
  ...
  const { data, isLoading: isBrandsLoading, isError, error } =
  ...
  const permission = useMemo(() => roleData?.data?.permissions?.brand, [roleData]);

  // Check if data or permissions are still loading
  if (isBrandsLoading || isRoleLoading) {
