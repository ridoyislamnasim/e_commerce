...
const shippingMethodRepository = require("./shipping.method.repository.js");
...
class ShippingMethodService extends BaseService {
  ...
  async createShippingMethod(payload, session) {
    ...
    const { shippingMethod, rate, status } = payload;
    // if (
    //   !header ||
    //   !title ||
    //   !description ||
    ...
    const shippingMethodData = await this.#repository.createShippingMethod(
      payload,
      session
    );
  ...
  async getShippingMethodWithPagination(payload) {
    const shippingMethod =
      await this.#repository.getShippingMethodWithPagination(payload);
    return shippingMethod;
  }

  async getSingleShippingMethod(id) {
    const shippingMethodData = await this.#repository.findById(id);
    if (!shippingMethodData) throw new NotFoundError("ShippingMethod Not Find");
    return shippingMethodData;
  }
  ...
  async updateShippingMethod(id, payload) {
    ...
    const shippingMethodData = await this.#repository.updateShippingMethod(
      id,
      payload
    );
    ...
    //   const filesToRemove = [
    //     requiredFiles.image ? shippingMethodData.image : null,
    //   ].filter(Boolean);
  ...
  async updateShippingMethodStatus(id, status) {
    if (!status) throw new NotFoundError("Status is required");
    status = status === "true";
    const shippingMethod = await this.#repository.updateShippingMethodStatus(
      id,
      {
        status: status,
      }
    ...
    if (!shippingMethod) throw new NotFoundError("ShippingMethod not found");
    return shippingMethod;
  }

  async deleteShippingMethod(id) {
    const shippingMethod = await this.#repository.findById(id);
    if (!shippingMethod) throw new NotFoundError("ShippingMethod not found");
    const deletedShippingMethod = await this.#repository.deleteById(id);
    console.log("shippingMethod", shippingMethod);
